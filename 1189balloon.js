...
var balloonN1 = function (text1) {
  ...
  return Math.min(
    ...
    Math.min(
      ...
      Math.min(
        Math.floor(arr[11] / 2),
        Math.min(Math.floor(arr[14] / 2), arr[13])
      )
    )
  );
};
...
//Math.min 和Math.max()可以跟多个参数，
//比如：  return Math.min(arr[1],arr[0],Math.floor(arr[11]/2),Math.floor(arr[14]/2), arr[13]);
//java 中的Math.min/max 只能两个参数
